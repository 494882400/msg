...
import com.msg_service.msg.config.DruidProperties;
...
public class YamlDynamicDataSourceProvider implements DynamicDataSourceProvider {
     ...
     */
    @Override
    public Map<String, DruidDataSource> loadDataSources() {
        //创建一个和配置文件数据源相同大小的map
        Map<String, DruidDataSource> ds = new HashMap<>(druidProperties.getDs().size());
        try {
